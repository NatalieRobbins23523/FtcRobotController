...
@TeleOp
public class DriveMainFieldCentricBlue extends LinearOpMode {
...
//            backRight.setPower(backRightPower);
            ...
            double yRobot = x*Math.cos(Math.toRadians(myHeading)) + y*Math.sin(Math.toRadians(myHeading)); //normally xRobot
            double xRobot = x*Math.sin(Math.toRadians(myHeading)) + y*Math.cos(Math.toRadians(myHeading)); //normally yRobot
            ...
            // reset heading when robot is facing blue goal side
    ...
    private void initializeOdometry(GoBildaPinpointDriver odometer) {
        ...
        Pose2D pose = new Pose2D(INCH, 0, -17, AngleUnit.DEGREES, -90); //usually 45 (angles are in odometry, ie. x axis is 0 degrees, so add 90 degrees)
